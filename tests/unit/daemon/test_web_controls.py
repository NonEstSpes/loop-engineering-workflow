...
# ---------------------------------------------------------------------------
# GET /api/providers + PUT /api/agents/{name} (Enhancements A)
# ---------------------------------------------------------------------------
...
def test_put_agent_updates_provider_model_temperature(tmp_path: Path) -> None:
    app, _ = _make_app_with_agents(tmp_path)
    with TestClient(app) as client:
        resp = client.put(
            "/api/agents/planner",
            json={"provider": "kimi", "model": "GLM-5.2", "temperature": 0.5},
        )
    assert resp.status_code == 200
...
def test_put_agent_unknown_404(tmp_path: Path) -> None:
    app, _ = _make_app_with_agents(tmp_path)
    with TestClient(app) as client:
        resp = client.put("/api/agents/bogus", json={"model": "x"})
    assert resp.status_code == 404


def test_put_agent_partial_update_only_model(tmp_path: Path) -> None:
    app, _ = _make_app_with_agents(tmp_path)
    original_provider = app.state.cfg.agents["planner"].provider
    with TestClient(app) as client:
        resp = client.put("/api/agents/planner", json={"model": "new-model"})
    assert resp.status_code == 200
    assert app.state.cfg.agents["planner"].model == "new-model"
    # Provider unchanged.
    assert app.state.cfg.agents["planner"].provider == original_provider


# ---------------------------------------------------------------------------
# tasks.updated SSE event (Enhancements A)
# ---------------------------------------------------------------------------

def test_tasks_updated_sse_event_published() -> None:
    """_write_todo_file publishes tasks.updated via event_bus when provided."""
    import asyncio
    ...
    cfg = Config(
        workflow=WorkflowConfig(task_source="mock", todo_path="TASKS.md"),
        providers={},
        agents={},
    )
    cfg.workflow.daemon = DaemonConfig(enabled=True, port=8787)
